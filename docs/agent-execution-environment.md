...
普通 `bash` 以及 read/edit/write 的实际文件操作使用
`@anthropic-ai/sandbox-runtime@0.0.75`。当前生产入口仅启用已验证的 macOS 后端；
其他平台保持失败关闭。`privileged_bash` 审批通过后走原生执行器，不添加 Pine 沙箱。
...
- 系统服务：采用 SRT 的受限 Mach 服务集合；不额外开放 Mach wildcard、Apple Events
  或 weaker network/nested sandbox 开关。
...
监督进程自己需要短路径控制 socket，使用 `/private/tmp/pine-srt-*` 的随机私有目录；
该目录不加入子进程的读写授权，正常退出时清理。控制平面与项目 scratch 分离，避免
长项目路径超过 Unix socket 长度限制。
...
普通 shell 传入最小环境，保留 HOME 和登录 PATH，将 TMPDIR、TMPPREFIX、缓存变量
指向项目 scratch。HOME/PATH 不隐式授予私有配置权限。原生执行保留宿主环境。
...
控制。Linux/Windows 未做实机验收，不宣称跨平台支持。
